...
OLD=OUT
OUT=ROOT/'data/processed/gait_frame_v1'
...
def main():
    ...
    manifest={str(p.relative_to(ROOT)):sha(p) for p in [Path(__file__),ROOT/'docs/classification/GAIT_FRAME_PROTOCOL.md',OLD/'windows.npy',OLD/'splits.csv',ROOT/'data/processed/gait_cnn_duogait_v2/cycles.csv']}
    ...
    datasets={'magnitude':(raw[:,:,[18]],ext[:,:,[6]]),'frame6':(frame,eframe)}
    ...
    gates=[]
    for (scope,seed),g in m[m.rule.eq('calibrated')].groupby(['scope','seed']):
        g=g.set_index('arm');a,b=g.loc['magnitude'],g.loc['frame6'];gates.append(dict(scope=scope,seed=int(seed),passed=bool((a.neuro_positive-b.neuro_positive)/a.neuro_n>=.05 and b.stroke_positive>=a.stroke_positive and b.healthy_positive<=a.healthy_positive)))
    (OUT/'decision.json').write_text(json.dumps(dict(gates=gates,passed=all(x['passed'] for x in gates),fits=36),indent=2));print(m[m.rule.eq('calibrated')].to_string(index=False))
