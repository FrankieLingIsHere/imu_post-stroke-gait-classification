...
The hands-free native flow now announces baseline and short-movement-check completion, asks the user to stop and stand still before advancing, and shows a progress cue during setup. The pre-walk hold is nine seconds with translated five-to-one countdown prompts emitted at the same one-second boundaries as the displayed timer; each short cue interrupts stale speech. Recording remains movement-gated and completion persistence is delayed briefly so the short stop cue can finish. Malay and Chinese catalog entries cover every new phrase. Automated TypeScript and 76-test validation pass.
...
## 2026-09-23 — Movement gate simplified
