...
export function exportFeatureCSV(session: SessionRecord): string {
  const rows: unknown[][] = [['session_id','source','feature_version','feature','value','unit','status','definition','reason','aggregation','placement_instruction','placement_verified','comparison_eligibility','capture_notes','estimator_version']];
  if (session.recording) {
    ...
    for (const [name, feature] of Object.entries(result.features)) {
      ...
      rows.push([session.id,'device',result.version,name,feature.value,feature.unit,feature.status,feature.definition,feature.reason,timing?'candidate-step-bouts-native-time':result.aggregation,result.placementInstruction,false,result.comparisonEligibility,result.captureNotes.join(' | '),timing?result.gaitTiming.version:result.version]);
    }
...
export function exportCSV(session: SessionRecord): string {
  const header = ['session_id', 'source', 'practice', 'started_at', 'planned_seconds', 'recorded_seconds', 'stop_reason', 'placement', 'coordinate_frame', 'sensor', 'units', 'requested_hz', 'elapsed_ms', 'received_at_unix_ms', 'sensor_timestamp_seconds', 'x', 'y', 'z', 'placement_review_required'];
  ...
  if (r) for (const name of SENSOR_NAMES) for (const s of r.streams[name]) rows.push([
    ...
    s.elapsedMs, s.receivedAtUnixMs, s.sensorTimestampSeconds, s.x, s.y, s.z, r.guidanceEvents.some(e => e.type === 'possible-placement-shift'),
  ]);
